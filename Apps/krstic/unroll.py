...
class TraceStep(object):
    ...
    def BuildFrame(self, lastTraceStep):
        ...
        if STRICT_TEMPLATE:
            self.decodeExpr = decodeExpr # z3.And( lastTraceStep.decodeExpr , decodeExpr) # instead of just decodeExpr
        else:
    ...
    def unroll(self):
        ...
        for threadProgram in self.TsPOList:
            for tsIdx in range(1,len(threadProgram)):
                threadProgram[tsIdx].BuildFrame(None)#threadProgram[tsIdx-1])

        # additionally add on the same entity : no two run at the same time


        mm.addHB_noOrder(self.ConstraintList, self.TsPOList)  #mm.addHB(self.ConstraintList, self.TsPOList)
...
# so such PC = and ... = 
    ...
    def compareModel(self): # assuming we already have the model
        fin = open('t.res')
        tmpSolver = z3.Solver()
        for line in fin.xreadlines():
            ...
            name = wd[0].strip()
            value = wd[1].strip()
            if ( name[0] == 'n' or name == 'rp' ) and value in ['True','False']:
                continue # skip n???: True
            if value in ['True','False']:
                print name,value
                assert False
            tmpSolver.add( z3.Int(name) == int(value) )
        assert tmpSolver.check() == z3.sat
        md = tmpSolver.model()

        # Now let's compare with our constraints
        print >> self.logcat, '========= DEBUG ========'
        idx = 0
        for c in self.ConstraintList:
            evalRes = str(md.eval(c))
            if evalRes != 'True':
                print >> self.logcat,'!!! --> ', idx
                #print >> self.logcat,c
                print >> self.logcat, evalRes
                # now let's add this to second round check

                if '(' not in evalRes and ')' not in evalRes and ( evalRes.startswith('pi_') or evalRes.startswith('def_') or evalRes.startswith('decode_') ):
                    if evalRes.count('==') == 1:
                        name = evalRes.split('==')[0].strip()
                        value = evalRes.split('==')[1].strip()
                        tmpSolver.add( z3.Int(name) == int(value) )
            idx += 1
        assert tmpSolver.check() == z3.sat
        md = tmpSolver.model()
...
# this is a dirty fix
CrossThreadLocal = ['active','DevIdx1','DevIdx2','DriPC','CEIdx','DevPC']

def test_main():
    import OoO as soc
    #import soc
    NoThread = (2,2,2) #NoThread = (2,2,1);
    ...
    #runProperty = lambda state: state('DriPC') == 2 
    #frameCond   = lambda ts: ts.entity == 'driver:'
