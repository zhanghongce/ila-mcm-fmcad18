...
class TraceStep(object):
    ...
    def __init__(self,inst, SysSharedState, tid = 0, TsThatWritesVar = None, TsNameToObj = None, ConstraintList = None, PiVarList = None, TsThatReadsVar = None) : # tell me which instruction to execute
        assert (TsThatWritesVar is not None and TsNameToObj is not None 
            and ConstraintList is not None and PiVarList is not None 
            and TsThatReadsVar is not None )
        
        self.TsThatWritesVar = TsThatWritesVar
        self.TsThatReadsVar  = TsThatReadsVar
...
class unroller(object):
    ...
    def unroll(self):
        ...
        globalBeginEvent = InitStep( self.SysSharedState , tid = 0, overwriteTs = 0, TsThatWritesVar = self.TsThatWritesVar, TsNameToObj = self.TsNameToObj)
        ...
        for Eidx in range(self.NumOfEntity): # entity idx  # global synchronous reset and local reset!
            ...
            for noRound in range(nTh):
                ...
                tid += 1
                self.TsPOList[-1].append( InitStep(self.TemplateProgram[Eidx][0].stlocal, tid, overwriteTs = 0, TsThatWritesVar = self.TsThatWritesVar, TsNameToObj = self.TsNameToObj ) ) # maybe we should use a synchronous init event?
                tsCnt += 1
    ...
    def finalizePiFun(self, rf_force = {} , model = None): # rf_force is a dict of rf-relation: read (KEY) <- write
        # print log
        ...
        for sname, piVar, ts in self.PiVarList.values(): # k
            ...
            if read_relId_tuple in rf_force_local:
                ...
                for traceStep in writerTraceSteps: # i
                    ...
                    # -.- Intercept on this Pi Function
                    if model is not None and ts.name == 'ReceiveReset_ts22' and ( sname == 'DevIdx1' or sname == 'DevIdx2' ) :
                        print >> self._logcat() ,'##### \n\tIntecept on ', ts.name,'state:', sname
                        print >> self.logcat, '\tread from ts: ',traceStep.name
                        print >> self.logcat, '\tconstraint:', constraint
                        print >> self.logcat, '\t<',model.eval(constraint)
            ...
            if len(OrClauses) == 0:
                ...
                disjConst = OrClauses[0]
            else:
                disjConst = z3.Or(OrClauses)

            if ONLY_CONSTRAINT_ON_READABLE_PI:
                disjConst = z3.Implies(ts.decodeExpr, disjConst)  
            self.ConstraintList.append( disjConst )
...
# so such PC = and ... = 
    ...
    def _logcat(self):
        if self.logcat is not None:
            return self.logcat
        self.logcat = open("log/modelCompare.log",'wt') 
        return self.logcat
    ...
    def buildRunProp(self, func = z3.And, model = None):
        self.runTProp = func(self.runConstraintList)
        print >> self.logcat, 'runProp :', self.runTProp   
        if model is not None:
            print >> self.logcat, '<<',model.eval(self.runTProp)   , '>>'  

    def pushInstConstraintAndCheck(self, model = None):
        ...
        for c in self.ConstraintList:
            if DEBUG:
                ...
                if model is not None:
                    print >> self.logcat, '<<', model.eval(c) , '>>' 
            #raw_input()
...
def Second(model):
    ...
    unroll.unroll()
    unroll.addPropertyOnSingleFrames( runProperty = runProperty, frameCond = frameCond)
    unroll.finalizePiFun(model = model)
    unroll.buildRunProp(func = z3.Or, model = model)
    unroll.pushInstConstraintAndCheck(model = model)
    #unroll.printModel()
    unroll.solve()
    return unroll.model
