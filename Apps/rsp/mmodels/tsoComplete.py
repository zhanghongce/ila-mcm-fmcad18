# This is the specification of Sequential Consistency (TSO) with fences, Yes Fences
import z3
...
class wvop(object):
    def _entity(self):
        return self.entity
    def __init__(self, ts, tsDefsGroup, TsThatWritesVar, ConstraintList ):
...
def createWriteView(ts, TsNameToObj, TsThatWritesVar, ConstraintList):
    ...
    wvop_ext = wvop(ts, tsDefsGroup, TsThatWritesVar, ConstraintList)
...
def addHB(ConstraintList,TsPOList):
    ...
    for tid1 in range(len(TsPOList)):  # same entity cross thread
        for tid2 in range(len(TsPOList)):
            if tid1 == tid2:
                continue
            if TsPOList[tid1][0]._entity() != TsPOList[tid2][0]._entity():
                continue
